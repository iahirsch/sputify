...
export default {
    ...
    methods: {
        async fetchMessage() {
            try {
                const response = await fetch('https://astonishing-llama-76e306.netlify.app/');
        ...
        async authorize() {
            try {
                const response = await fetch('https://astonishing-llama-76e306.netlify.app/authorize', { credentials: 'include' });
                if (response.ok) {
